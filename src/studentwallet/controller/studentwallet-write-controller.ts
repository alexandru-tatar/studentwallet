...
import {
    StudentCreate,
    type StudentFileCreated,
    StudentUpdate,
    StudentwalletWriteService,
} from '../service/studentwallet-write-service.js';
import { createBaseUri } from './create-base-uri.js';
import { InvalidMimeTypeException } from './exceptions.js';
import { StudentDTO, StudentDtoOhneRef } from './student-dto.js';
...
export class StudentWriteController {
    readonly #service: StudentwalletWriteService;

    readonly #logger = getLogger(StudentWriteController.name);

    constructor(service: StudentwalletWriteService) {
    ...
    async post(
        ...
        const student = this.#studentDtoToBuchCreateInput(studentDTO);
    ...
    async addFile(
        ...
        const studentFile: StudentFileCreated | undefined =
            await this.#service.addFile(id, buffer, originalname, size);
        this.#logger.debug(
            'addFile: id=%d, byteLength=%d, filename=%s, mimetype=%s',
            studentFile?.id ?? -1,
            studentFile?.data.byteLength ?? -1,
            studentFile?.filename ?? 'undefined',
            studentFile?.mimetype ?? 'null',
        );
    ...
    async put(
        ...
        this.#logger.debug(
            'put: id=%d, stundetDTO=%o, version=%s',
            id,
            studentDTO,
            version ?? 'undefined',
        );
        ...
        const student = this.#studentDtoToStudentUpdate(studentDTO);
        const neueVersion = await this.#service.update({ id, student, version });
    ...
    #studentDtoToBuchCreateInput(studentDTO: StudentDTO): StudentCreate {
    const transactions = (studentDTO.transactions ?? []).map((t) => ({
        amount: new BigNumber(t.amount).toNumber(),
        type: t.type,
        ...(t.reference != null && { reference: t.reference }),
        ...(t.location  != null && { location:  t.location  }),
        ...(t.recordedAt != null && {
        recordedAt: typeof t.recordedAt === 'string' ? new Date(t.recordedAt) : t.recordedAt,
        }),
    }));

    const student: StudentCreate = {
        version: 0,
        matriculationNumber: studentDTO.matriculationNumber,
        ...
        wallet: {
        create: {
            balance: new BigNumber(studentDTO.wallet.balance ?? 0).toNumber(),
            autoReloadEnabled:
            studentDTO.wallet.autoReloadEnabled ??
            (studentDTO.wallet.autoReloadAmount != null &&
            new BigNumber(studentDTO.wallet.autoReloadAmount).gt(0)),

            ...(studentDTO.wallet.autoReloadThreshold != null && {
            autoReloadThreshold: new BigNumber(studentDTO.wallet.autoReloadThreshold).toNumber(),
            }),
            ...(studentDTO.wallet.autoReloadAmount != null && {
            autoReloadAmount: new BigNumber(studentDTO.wallet.autoReloadAmount).toNumber(),
            }),

            ...(studentDTO.wallet.lastReloaded != null && {
            lastReloaded:
                typeof studentDTO.wallet.lastReloaded === 'string'
                ? new Date(studentDTO.wallet.lastReloaded)
                : studentDTO.wallet.lastReloaded,
            }),
        },
        },
    ...
    #studentDtoToStudentUpdate(studentDTO: StudentDtoOhneRef): StudentUpdate {
        return {
            version: 0,
            matriculationNumber: studentDTO.matriculationNumber,
            firstName: studentDTO.firstName,
